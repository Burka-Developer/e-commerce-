...
export default function ProductDetailPage() {
  ...
  const productId = Number.parseInt((params as any)?.id ?? "0")
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
          <Link href="/" className="hover:text-primary hover-underline focus-ring rounded">
            Home
          </Link>
          <span>/</span>
          <Link href="/products" className="hover:text-primary hover-underline focus-ring rounded">
            Products
          </Link>
          <span>/</span>
          <Link href={`/categories/${product.category.toLowerCase()}`} className="hover:text-primary hover-underline focus-ring rounded">
            {product.category}
          </Link>
          <span>/</span>
          ...
          <div className="space-y-4">
            <div className="group relative aspect-square overflow-hidden rounded-lg border">
              <Image
                ...
                width={500}
                height={500}
                className="w-full h-full object-cover cursor-zoom-in transition-transform duration-300 ease-out group-hover:scale-110"
              />
              {/* subtle zoom indicator */}
              <div className="pointer-events-none absolute bottom-2 right-2 rounded-md bg-background/70 px-2 py-1 text-[11px] text-muted-foreground shadow-sm">Hover to zoom</div>
            </div>
        ...
        <div className="lg:hidden fixed inset-x-0 bottom-0 z-40 border-t bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="container flex items-center gap-3 py-3">
            ...
            <Button className="flex-1" onClick={handleAddToCart} disabled={!product.inStock}>
              <ShoppingCart className="h-5 w-5 mr-2" />
              {product.inStock ? `Add • $${(product.price * quantity).toFixed(2)}` : "Out of Stock"}
            </Button>
          </div>
        </div>
